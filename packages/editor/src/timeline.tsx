...
export function Timeline({ video }: { video: RefObject<HTMLVideoElement | null> }) {
  const { bundle, playheadMs, selectEvent, hoverEvent, setPlayhead, setBound } = useEditorStore();
  ...
  return <section className="timeline" aria-label="Recording timeline" tabIndex={0} onKeyDown={(event) => {
    ...
    <div className="trace-lane" aria-label="Trace events">
      {bundle.events.map((event, index) => { const mediaTime = bundle.clock.toMediaTime(event.t); return <button key={event.id} className={`tick tick-${event.type.split('.').at(-1)}`} style={{ left: `${Math.max(0, Math.min(100, mediaTime / duration * 100))}%`, top: `${4 + tickRow(index) * 9}px` }}
